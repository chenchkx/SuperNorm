            ...
            for i in range(len(A_array)):
                ...
                if len(subgraph_nodes) == 1:
                    ...
                    for eig_th in range(len(S)):
                        q = S[eig_th].item()
                        eig_factor = eig_factor + q*0.1**eig_th  

                    sg_dense = 2*len(subgraph_edges)/(len(subgraph_nodes)*(len(subgraph_nodes)-1))

                    sg_factor[i] = sg_dense*len(subgraph_nodes)*len(subgraph_nodes) + len(subgraph_nodes)*0.1**2 + eig_factor*0.1**1
                    # sg_factor[i] = sg_dense*len(subgraph_nodes)*len(subgraph_nodes)
                    # sg_factor[i] = len(subgraph_nodes)**2
